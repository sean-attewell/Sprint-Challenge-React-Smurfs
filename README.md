...
Server-side routing will usually request a whole new page to be served to the user, causing the whole page to refresh. Often unnecessary data is being requested such as a header or footer that stays the same between pages.

Client-side routing is handled internally by the JavaScript and allows the construction of single page applications. While the URL may change, the request to the server is prevented. The adjustment to the URL will result in a changed state and subsequently a different view of the page. E.g. the rendering of a new component. The downside is that the whole website needs to be loaded on the first request, even if the user doesn’t end up seeing it all.
